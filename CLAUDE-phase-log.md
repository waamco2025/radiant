...
### Phase 18.0.1 completion notes (2026-05-21) — Hotfix: sticky edge tooltip on Parent → Directory transition (closes #203)

Demo-blocking bug Andrew hit while recording the RFP walkthrough: the parent-canvas "SELECT EDGE TO VIEW" hover tooltip persisted over the Directory layer after transitioning Parent → Directory.

**Root cause.** Three state layers drive parent-canvas edge tooltips: (1) V2Canvas-internal `hoveredEdge`/`edgeTooltipPos`/`hoveredEdgeSdaType` (the small SDA cursor dot, z-index 5900); (2) V2App `edgeHover` (the `EdgeHoverMenu` hover-mode "SELECT EDGE TO VIEW" card, z-index 6000, portaled to document.body); (3) V2App `edgeMenu` (the pinned-mode menu). Phase 17.4.3's `clearEdgeUiState()` clears all three on the Directory-open transition correctly + synchronously. The bug was post-clear: V2Canvas's `onMouseMove`-bound `handleEdgeHover` had only a `transitioningRef.current` early-return (V2Canvas-internal dive/surface transitions) — no signal that the Directory was open. So during/after the ~600ms clip-path wipe, any mousemove re-raycasted parent edges, re-fired `onEdgeHover({ edgeId, sdaType, x, y })`, and re-set V2App's `edgeHover` → the `EdgeHoverMenu` re-rendered over the Directory. The "intermittent" #203 framing was because repro needs the cursor over a hit-range edge + any mousemove post-clear — both common during recording.

**Diff (high-level).**
- **V2Canvas** (`src/v2/V2Canvas.jsx`): new `directoryOpen = false` prop in the `forwardRef` destructure. `handleEdgeHover` and `handleBackgroundClick` each get an `if (directoryOpen) return` early-return next to the existing `transitioningRef.current` check (both useCallback dep arrays gain `directoryOpen`). New `useEffect([directoryOpen])` that, when `directoryOpen` is true, clears `hoveredEdge`/`edgeTooltipPos`/`hoveredEdgeSdaType` and cancels the pending 150ms `edgeHideTimeout` debounce — belt-and-suspenders for the small SDA cursor dot + a stale hide-timer that could otherwise fire `onEdgeHover?.(null)` after the Directory is open.
- **V2App** (`src/v2/V2App.jsx`): passes `directoryOpen={v22DirectoryOpen}` to `<V2Canvas>`. Both `EdgeHoverMenu` mount conditionals (pinned-mode `{edgeMenu && …}` and hover-mode `{!edgeMenu && edgeHover && …}`) gained a leading `!v22DirectoryOpen &&` — defensive backstop so even a stray `onEdgeHover` after the transition can't mount the menu over the Directory. `clearEdgeUiState` extended with `setEdgeMenuPanning(false)`.
- **Docs + version:** footer + Changelog modal → v0.18.0.1; architecture-spec §8 Changelog bullet; polish-backlog #203 marked ✅ Complete + moved to the Edge Interactions Completed section + Update Log entry; CLAUDE.md most-recent-phase + queue.

**Deviations.** The prompt's Part 2C said to add `setEdgeMenuProjected(null)` to `clearEdgeUiState`, but no such state exists in the codebase. The pinned menu re-projects `edgeMenu.anchor` in place via a fade effect keyed on `edgeMenu?.edgeId`, with `edgeMenuPanning` as the fade/world-space-tracking flag (and that effect already resets `edgeMenuPanning` to false when `edgeMenu` goes null). So `setEdgeMenuPanning(false)` is the real-world equivalent of the prompt's intent (purely belt-and-suspenders for completeness). **Audit:** all Directory-open paths (globe toggle line 5036, Radiant-Network double-click 6140, RFP-notification routing 5547, role switch, `routeToRfp` 4508, `routeToAsset` exit) already call `clearEdgeUiState`; with the V2Canvas guard + the V2App mount gate, a missed clear is now harmless — no path needed adding.

**Runtime verification.** Build clean (132 modules, 0 errors). Dev preview (Bob, 1280×800): opened the Directory (globe), then dispatched ~900 mousemoves over the parent canvas while the Directory was open → no "SELECT EDGE TO VIEW" tooltip appeared, no console errors (exercises the new `directoryOpen` early-return in `handleEdgeHover`); closed the Directory (exit chip) → clean return to Parent, subsequent mousemoves produced no stale tooltip; no console errors across the cycle. **Flagged (documented limitation):** the actual hover-tooltip *repro* (hover an edge → tooltip → open Directory) is canvas-raycaster-gated — DOM-dispatched mousemoves don't trigger the Three.js edge raycaster (CLAUDE.md V2Canvas raycaster limitation; a fine 40×24 mousemove sweep produced no edge hit), so the before/after couldn't be empirically demonstrated in-session. The fix is a straightforward early-return guard + render gate + state-clear effect (low-risk, code-verified) backed by the verified-clean open/close cycle. **#203 closed.**

**Status:** [x] Complete.

### Phase 18.0 completion notes (2026-05-21) — Demo prep quick wins (selection ring + View File button + amend edge animation + Eval/Parse Result Agreements)

First demo-prep pass before the demo walkthrough recording. Four S-effort, visible-to-the-eye polish items. Phase 17's RFP creation arc is fully closed; Phase 18 opens as the demo-prep umbrella.

**Diff (high-level).**
- **Part 1 — Selection ring centering** (`src/v2/AssetNode.jsx`, `AssetNodeDot`): the amber `isSelected` ring (a child of the inner 8×8 dot div, which has a 1.5px border + `position: relative`) used `top: -6, left: -6`. Absolute `top`/`left` resolve against the containing block's *padding edge* — inside the 1.5px border — so the 20×20 ring's center landed ~1.5px down-and-right of the dot's visual center (the reported "below-and-right" offset). Switched to `top: '50%', left: '50%', transform: 'translate(-50%, -50%)'`, which centers on the dot's visual center independent of border width.
- **Part 2 — View File button unification** (`src/components/DetailPanel/V22NodeDetailPanel.jsx`, `V22AssetPanel`): added a shared `viewFileButton` const (the large full-width button, `onClick = onExpandAsset(asset)`, label "View File"). The owner File section now renders its metadata rows (Filename/Size/MIME/Hash/URI — owner-only per Phase 11D #135) + the button, dropping the small `ExpandButton` header-action icon. The non-owner File section renders just the button (was "Open Evidence Viewer"). `ExpandButton` import retained (still used by the Claim panels).
- **Part 3 (#35) — Amend Claim edge animation** (`src/v2/v2_2Data.js` + `src/v2/V2App.jsx`): `makeAmendedClaim`'s `newClaimRefEdges` now spread `_isNew: true` + `_createdAt: new Date(amendmentDate).getTime()` onto each new claim-ref DA. `deriveAgreementEdges`'s `pushEdge` propagates `_isNew` + `_createdAt` from the source DA onto the derived edge (V2Canvas's `animateEdgeDraw` zeroes then grows a `_isNew` Line2's `instanceCount`, gated to a 3s `_createdAt` window). Because claim-ref DAs are permanent internal DAs (unlike transient `e-prov-` provisional edges), `handleV22AmendClaimSubmit` schedules a ~1.1s `setTimeout` that clears `_isNew`/`_createdAt` from the just-added DA ids (`da-ref-{claimId}-{assetId}`) — so the edge animates once then returns to normal thin-internal styling instead of staying brighter/thicker. Scoped to the amendment's own DA ids; no cross-impact on the provisional reveal path.
- **Part 4 (#116) — Agreements section on Eval/Parse Result panels** (`src/components/DetailPanel/V22NodeDetailPanel.jsx` + `src/v2/V2App.jsx`): V2App's parent-canvas Detail Panel agreement-filter gained two branches — `EVAL RESULT` → DAs where `subject.kind === 'evalResult' && subject.id === node.id`; `PARSE RESULT` → `subject.kind === 'parseResult'` (empty in V2.2). `V22EvalResultPanel` + `V22ParseResultPanel` destructure the existing agreement props (already passed to `V22NodeDetailPanel` at the mount + spread `{...props}` by the router) and render `<AgreementsSection>` before their body close. `AgreementsSection` returns `null` when empty, so the Parse Result section renders nothing (matching 9C). No new primitives.
- **Docs + version:** footer + Changelog modal → v0.18.0; architecture-spec §8 Changelog bullet; polish-backlog Update Log + #35/#116 marked ✅ Complete and moved to the Completed section (Edge Interactions / Detail Panels); CLAUDE.md most-recent-phase + queue (Phase 17 wrapped, Phase 18 demo-prep umbrella).

**Deviations.** (a) Part 1: rather than the literal `top:-6/left:-6` author-intent (correct against the border box, off against the padding box), used `50%`/translate centering — robust against the border offset and the idiomatic centering pattern the prompt suggested. (b) Part 2: kept the owner's file-metadata rows (the prompt's "remove the small expand-icon + FILE label pattern" targets the expand-icon affordance; the metadata is legitimate owner-only data per Phase 11D #135) — surfaced as a deviation per the prompt's "if the FILE label is part of a different section, confirm the canonical action" guidance. (c) Part 3: the prompt framed this as a 1-line `_isNew` tag, but `_isNew` also drives *permanent* edge styling (brighter + 3px, bypassing the 0.5× internal-edge thinning), not just the `_createdAt`-gated animation — so a persisted flag would leave the amended edge highlighted forever. Added the numeric `_createdAt` (re-animation gate) + the ~1.1s reveal-flag clear-timer to make `_isNew` transient, scoped to the amendment flow. (d) Part 4: the eval-result-subject DA row shows the eval-result *id* as its subject name — the shared `resolveSubjectName` returns `artifact.name`, which eval-result artifacts don't carry; improving that resolver is out of the "no new primitives" scope, noted for a future detail-panel polish pass.

**Runtime verification.** Build clean (132 modules, 0 errors). Dev preview (Bob, 1280×800): Part 1 — selected a dot-LOD node and measured the ring center == inner-dot center (offsetX 0, offsetY 0, concentric); Part 2 — own-Asset panel shows the 443px full-width "View File" button (no "Open Evidence Viewer", no expand icon, metadata rows preserved) and clicking it opens the evidence viewer modal; Part 4 — Bob's "Evaluation of Power Regulation Module Assembly" panel renders AGREEMENTS → DISCLOSURE AGREEMENTS (1) with the eval-result-subject DA row. No console errors across the flows. **Flagged for manual verification** (headless preview's rAF throttle / data-model norms, per the documented Three.js/preview limitation): Part 3's edge-draw *animation* (rAF-paced — the data/propagation path + clear-timer are build-clean and follow the verified provisional-edge contract); Part 2's non-owner branch (renders the identical shared `viewFileButton` element — owner branch confirmed live); Part 4's Parse Result section (empty-by-design — `AgreementsSection` returns null; props threaded with defaults, build-clean, no error path).
...
**Diff (high-level).**
- **Part 1 — Tab-crash regression** (`src/v2/V2App.jsx`): the 17.5.1.5 Anchored RFPs row click locked the tab. Root cause: `routeToRfp`'s rAF select-retry chain (`trySelect`) was never cancelled when `routeToRfp` was re-invoked, and 17.5.2 raised its cap 60 → 240 frames (~4s @ 60fps). The no-opts `routeToRfp` body was otherwise byte-identical to 17.5.1.5 — the cap bump + the absence of cancellation is the only behavioral delta. So repeated RFP navigation (Anchored-RFPs click → For-Asset round trip → click again, rapid re-clicks) stacked multiple concurrent 4-second chains, each filtering the ~23k-entry `allDots` every frame and firing `setPinned`/`animatedPanToWithZoom` on resolution — saturating the main thread → perceived tab freeze. The retry loop itself is provably bounded (240-frame cap, not an infinite loop); the freeze is accumulation of overlapping bounded chains at 60fps. Fix: new `routeRfpSelectRafRef = useRef(0)`; `routeToRfp` `cancelAnimationFrame`s any in-flight chain before starting a new one and zeroes the ref on terminate (resolution or cap); `routeToAsset` cancels it too on the way back to Parent so a lingering chain can't fire on the closing Directory. Only one chain is ever live.
- **Part 2 — Smooth Directory → Parent transition** (`src/v2/DirectoryLayer.jsx`): new module const `DIRECTORY_EXIT_ZOOM = 0.6` + a `phase === 'out'` `useEffect` that fires `animatedPanToWithZoom(camPosRef.current.x, camPosRef.current.y, Math.min(zoomRef.current, 0.6), 900)`. The camera eases outward over the 900ms reverse circle wipe, in parallel with the Parent pre-positioning `routeToAsset` already fires (`canvasRef…animatedPanToWithZoom(asset.x, asset.y, 1.0, 500)` — the established `handleViewEa` convention). `Math.min` keeps it outward-only (closing from an already-wider view doesn't zoom in). Gating on `phase === 'out'` applies it to every close path (globe button, exit chip, double-click escape, For-Asset route). The scene unmounts at phase `'closed'` (~600ms), cutting the easeOut tail (movement is front-loaded). A near-no-op guard (`< 1e-4`) skips the animation when already at the target.
- **Part 3 — Anchored RFPs row restructure** (`src/components/DetailPanel/V22NodeDetailPanel.jsx`, `AssetPanelRfpRow`): dropped the `✕`/`⟲`/`⊟` icon prefixes from the CLOSE/REOPEN/REMOVE `ActionLabel` labels (text-only now). Switched the right region from a horizontal row (`alignItems: center, gap: 10`) to a vertical column (`flexDirection: column, alignItems: flex-end, gap: 4, minWidth: 52`) so REOPEN+REMOVE stack when both render (closed RFP), exactly matching the AMEND/REVOKE EA-row treatment. Open RFP shows a single CLOSE label in the same column. REMOVE keeps its `danger` (red) variant.
- **Parts 4+5 — ASSET / ACTOR header badges** (`V22AssetPanel`, `V22ActorPanel`): both now pass `typeLabel="ASSET"` / `typeLabel="ACTOR"` to `PanelHeader`, which already renders the standard TYPE_BADGE_BG mono-uppercase pill. Reverses the 17.3 #202 removal for these two panels so every Detail Panel header carries a type badge consistently. The PanelHeader comment was updated to record the reversal.
- **Part 6 — Changelog modal header copy** (`src/v2/V2App.jsx`): "Prototype Changelog" → "Changelog"; "Radiant V2 — PCN Prototyping" → "Radiant v2.2" (V2App only; the archived V3App copy is untouched).
- **Docs + version:** footer + Changelog modal → v0.17.5.2.1; architecture-spec §8 Changelog bullet; polish-backlog Update Log; CLAUDE.md most-recent-phase + queue (RFP arc fully CLOSED).

**Deviations.** The prompt attributed the crash to "17.5.2's routeToRfp refactor"; the diagnostic confirmed the no-opts path was byte-identical to 17.5.1.5, so the actual delta is the cap bump (60→240) + the long-standing lack of loop cancellation interacting under repeated navigation — documented precisely rather than blaming a single line. Part 2's "~60%" was implemented as `DIRECTORY_EXIT_ZOOM = 0.6` with `Math.min` (outward-only) per the prompt's "tune during QA / lower = wider" guidance.

**Runtime verification.** Build clean (132 modules, 0 errors). Dev preview (Bob): Part 1 — anchored Sentinel-4 RFP click routes cleanly (Directory opens, RfpDetailPanel renders, page responsive), with a clean For-Asset round trip back to the Asset panel and zero console errors; Part 3 — open RFP shows text-only `CLOSE`, after Close the row shows `REOPEN`/`REMOVE` stacked (measured `verticalGap: 4`, same right-aligned column) with REMOVE red (`rgb(239,68,68)`); Parts 4+5 — Asset panel header reads `ASSET …`, Actor panel header reads `ACTOR …`; Part 6 — Changelog modal reads "Changelog" / "Radiant v2.2" with the new v0.17.5.2.1 entry on top, footer reads v0.17.5.2.1. **Flagged for manual verification:** Part 2's camera zoom-out pacing — the headless preview throttles rAF to ~1fps (a self-rescheduling rAF loop never resolved within 30s), so rAF-driven animation timing cannot be confirmed in-session; the Parent pre-positioning + close path are confirmed and the zoom-out is wired into the verified `phase === 'out'` close effect. This is the documented Three.js/preview-throttling limitation — needs a 60fps foreground pass. The same throttling means Part 1's freeze (a 60fps-timing phenomenon) couldn't be reproduced in-session either; the fix is structural (one live chain max) and verified by clean repeat-navigation with no errors.

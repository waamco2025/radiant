...
End-of-Phase-11 architectural cleanup batch — items surfaced by Phase 11E QA that need to land before Phase 12 starts. Distinct from Phase 14's Directory Layer batch and Phase 15's broader UX polish; this queue is for issues that exposed gaps in the Phase 11 architecture itself.

### 164. Acknowledgment amendment acceptance UX (counter-acceptance flow)
- **Status:** Deferred to Phase 11.6 cleanup
- **Effort:** L
- **Source:** Phase 11E.4 QA — Andrew flagged that Phase 11E.1's Option B Amend EA semantics allow the grantor to unilaterally add, edit, or remove acknowledgments after the EA has been accepted. This is an exploit vector: the grantor could append "Bob agrees to pay $1M" post-acceptance and the grantee has no consent gate.
- **Architectural shift:** Acknowledgment changes via Amend EA must require explicit grantee re-acceptance, not just notification.
- **Proposed UX:**
  - Acknowledgment-changed Amend EA fires the existing `v22-ea-amendment` notification, but the notification's deep-link opens a new **AcknowledgmentReacceptanceModal** instead of the EA Detail Panel.
  - Modal lists each added / changed / removed acknowledgment with a diff-style display. Each row has a checkbox the grantee must tick.
  - Two outcomes: grantee checks all → Accept (re-adopts the amendment, EA continues active with new acknowledgments). Grantee Rejects → EA voided.
  - **Voiding semantics:** the EA is marked revoked (`status = 'revoked'`, `revokedReason = 'amendment-rejected-by-grantee'`). The grantee's Eval Results stay on their network (the work product persists). The grantor's Claim node disappears from the grantee's canvas (per existing EA-revocation flow). The underlying DA stays active — the grantee retains visibility into the Claim via the Directory Layer (per #162 architectural resolution).
- **Affects:**
  - `AmendEvaluationAgreementModal.jsx` — gate ack changes behind the knowledge that they trigger re-acceptance.
  - New `AcknowledgmentReacceptanceModal.jsx`.
  - `V2App.jsx` notification click handler for `v22-ea-amendment` — branch on whether the amendment included ack changes.
  - `v2_2Data.js` — extend amendment record shape to track re-acceptance state per grantee.
  - Architecture spec §11.2a — major update; the Option B "implicit propagation" limitation becomes an explicit-acceptance contract for ack changes.
- **Out of scope (still Option B implicit):** expiration changes, amendment notes only. Only acknowledgment changes trigger re-acceptance.
- **Depends on:** Phase 11.6 cleanup batch (the slot Andrew has reserved for end-of-Phase-11 architectural cleanup before Phase 12).

...
- **Source:** Phase 9A.6.2 QA — pre-existing, not introduced by this phase.
- **Scope:** Small.
- **Priority:** Low.
- **Context:** React warns at render time: "Updating a style property during rerender (border) when a conflicting property is set (borderTopColor)." Somewhere in `AssetNode.jsx` (likely in the selected-state or `_isEdgeEndpoint` visual-treatment blocks) a style object mixes `border: ...` shorthand with `borderTopColor: ...` (or similar longhand). Fires 3–5 times per render cycle. Non-fatal; styles still apply, but ordering is non-deterministic per React.
- **Proposed fix:** audit `AssetNode.jsx`'s style objects. Replace `border: ...` shorthand with four longhand properties (`borderWidth`, `borderStyle`, `borderTopColor`, `borderRightColor`, `borderBottomColor`, `borderLeftColor`) in any style object that also sets one of those longhand properties. Alternative: stop setting the longhand override; always set `border` shorthand with the final intended color.
...
- **Status:** Superseded by #111 (same intent, scoped out + rewritten with current conventions).
- **Source:** Phase 3 visual review
- **Scope:** Medium
- **Context:** Amend DA and Amend EA are currently only accessible by clicking edges. Users expect to be able to take these actions from the relevant node's Detail Panel too (e.g., "Amend disclosure for this Claim" from the Claim's panel). **See #111 for the current scoped version of this work.**

### 111. Agreements section in node Detail Panels (primary access path for Amend / Revoke)
- **Source:** Phase 9B.1 QA — Andrew clarified edge-click is the *secondary* access path; primary should be Detail Panels.
- **Scope:** Medium.
- **Priority:** **High.**
- **Context:** Every node that participates in Disclosure and Evaluation Agreements should surface those Agreements in its Detail Panel as a dedicated "Agreements" section. Each row shows the Agreement summary (type, counterparty, expiration) with Amend / Revoke action buttons on the right side. Amend opens the appropriate Amend modal (existing for DA; #108 pending for EA). Revoke initiates the revocation flow (#112). This is the natural UX path — users looking at a Claim, Asset, or Actor expect to manage its Agreements from that node's panel, not by hunting through edges. Supersedes backlog #12 (same intent, this is the scoped version).
- **Proposed fix:** Add Agreements section to V22NodeDetailPanel for CLAIM, ASSET, and ACTOR types. Derive the list from `v22View`'s `disclosureAgreements` + `evaluationAgreements` filtered to where this node participates.

### 112. Revocation flow restoration
- **Source:** Phase 9B.1 QA — V2.1 capability lost during V2.2 migration.
- **Scope:** Medium/Large.
- **Priority:** **High.**
- **Context:** V2.1 supported Agreement revocation. Owner-initiated revoke flow: confirmation dialog → removes Agreement from owner's canvas → counterparty notification. Counterparty-side effects: notification with pan/zoom to node, REVOKED badge on node card, Detail Panel shows REVOKED header + message explaining consequence + original sender's reason (or "No reason given" italic) + Dismiss button under the message. Dismiss removes node from counterparty's canvas. Self-revocation (removing own nodes from own network) also supported via same Revoke action. Revoked records persist in a log even after canvas dismissal. Andrew has V2.1 reference files in backups.
- **Proposed fix:** Build `V22RevocationModal.jsx` for confirmation + optional reason textarea. Add Revoke action to Agreements section (#111) and to node Detail Panel footers for self-revocation. Wire handlers in V2App for owner-side (remove from canvas, emit notification) and counterparty-side (add REVOKED badge, populate revocation message). Add `revocationRecords` to `v22Provisionals` state. REVOKED badge renders in AssetNode / relevant node components. Reference V2.1 `RevocationNoticeModal.jsx` (deleted in Phase 8 per backlog #14 — Andrew to place in `/reference/` before this phase starts).

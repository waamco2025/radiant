...
- **Status:** Under investigation (Phase 9A.6.2). Diagnostic instrumentation shipped at three layers: V22CreateClaimModal's `handleComplete` (logs `selected` Set contents + what's sent downstream), V2App's `handleV22CreateClaimSubmit` entry (logs received `referencedAssetIds` + factory output), and the Run Evaluation modal mount from 9A.6.1.1 (logs Claim + DA scope + rendered evidence ids). Exhaustive static code review found no bug in the four candidate paths (nested CTA overwrite, stale submit closure, factory ID drop, materialization race). Data-layer programmatic simulation with 5 seeded + 2 new Assets produces a Claim with `referencedAssetIds.length === 7` end-to-end — the happy path is clean at every layer. The UI-driven non-deterministic drop reported by Andrew was not reproducible through code review or static simulation. Instrumentation will surface the divergence on next reproduction. **Open for 9A.6.2.1 follow-up once a fresh reproduction + log capture is available.**
- **Source:** Phase 9A.6.1 QA — regression; rescoped in 9A.6.2.
- **Scope:** Medium
- **Priority:** **High — functional regression; fix blocked on reproduction-with-logs.**

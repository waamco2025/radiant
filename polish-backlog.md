...
- **Status:** ✅ Complete (Phase 9D.2). New `src/v2/animations/unravel.js` exports `playUnravelAnimation({ nodeId, canvasRef, setUnravelingNodeId, ensureFocused, onComplete })` returning a Promise. Stages: (0) pan/zoom to node (~400ms, skipped when already focused per `isFocusedOnPoint` tolerance check); (1) Three.js edge retract via new `playEdgeRetract` on V2Canvas's imperative handle — animates Line2 geometry per-frame so each curve point collapses toward the OPPOSITE endpoint with ease-out cubic + opacity tail-fade (~400ms); (2-4) CSS card unravel via new `node-unravel` keyframe in index.css — coordinated border + content + bg fade + slight settle translate (~900ms, starts at t=300ms overlapping Stage 1's tail). Total ~1.2s after pan. New V2Canvas methods: `getNodeWorldPos(nodeId)`, `isFocusedOnPoint(x, y, tolPx)`, `playEdgeRetract(nodeId, durationMs)`. New `v22UnravelingNodeId` state in V2App threaded through `v22DataWithReveal` → AssetNode reads `node._unraveling` to apply the keyframe. Wired into `handleV22DismissRevoked` (Case A Claim) + `handleV22DismissOrphanedEvalResult` (orphaned ER) — both now `async` and `await` the primitive before mutating state. Detail Panel close (`setSel(null)`) also moved past the await so the panel doesn't snap shut mid-animation. **Stage 2 fallback shipped per task brief allowance:** the original spec called for a clockwise dashed-border unwind — collapsed into the coordinated CSS keyframe. Building the clockwise unwind would have required either an SVG overlay tracking card pos+size through pan/zoom or a custom canvas2D layer; disproportionately expensive vs. the visual gain over a coordinated fade. Other "leaves the canvas" scenarios (transfer-accept on sender, expired agreements, explicit delete) can call `playUnravelAnimation` once they ship.
...
- **Context:** There is currently no path to create a new Evaluation Agreement against a pre-existing Disclosure Agreement. The CombinedRequestModal assumes both DA and EA are provisioned together. This means if an EA is revoked (Cases C/D) or expires while the paired DA remains active, the grantee has no way to reinstate evaluation capability without revoking and re-requesting the entire DA.
- **Proposed fix:** When a Claim viewed by a grantee has an active DA but no active EA, surface a "Request Evaluation Agreement" action in the footer of the Claim's Detail Panel. This opens a slim EA-only request modal (skipping the Disclosure step since the DA is already in place). The grantor receives a `v22-request` notification and responds via a slim response modal covering only EA terms (no disclosure-type selection, no scope-defining step).
- **Depends on:** Pairs with #113 (Split Combined Request into distinct Disclosure + Evaluation steps). The "warm path" (DA exists, just need EA) that #113 introduces is exactly this reinstate scenario — can likely ship as part of Phase 11 #113 work rather than a separate effort.

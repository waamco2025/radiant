...
- Color shifts toward white: the disclosure-type color is lightened by blending 40% with pure white, producing a brighter variant of the same hue.
- Stroke width increases from the default 1.5px to 2px.
...
- Bob does not see Alice's individual Assets — he only sees her Claim. He may see referenced Asset metadata inside the Claim's Detail Panel depending on the disclosure type.
...
1. **Bob** enters Alice's Claim PIN in the Request Disclosure flow from his Asset's Detail Panel footer.
2. Platform creates a provisional Disclosure Agreement + Evaluation Agreement pair. Alice's Claim appears on Bob's canvas as a provisional node (dashed border) with a provisional Agreement Edge.
3. Alice receives a notification.
4. **Alice** opens the combined Disclosure + Evaluation Agreement request modal (see §9.5). She sets disclosure type (Full/Selective/Proof-Only), scope (which Assets or parsed fields), Evaluation Agreement terms (which requirements sets authorized, deadlines, incentives), and accepts.
5. Platform finalizes both agreements. Alice's Claim transitions from provisional to active on Bob's canvas (reveal animation). Reciprocal Agreement Edges appear on both canvases.
6. **Bob** runs an evaluation from the Agreement Edge (or from Alice's Claim node footer). Platform confirms Bob is using one of the requirements sets authorized in the Evaluation Agreement.
7. Platform produces an Evaluation Result. It appears as a parent-layer node on Bob's canvas (connected to his Asset and to Alice's Claim) and on Alice's canvas (connected to her Claim).
8. If the result is unsatisfactory, Alice can amend her Claim (add more Assets) and amend the Disclosure Agreement with Bob to include those new Assets. Bob gets notified, re-runs the evaluation. The new Evaluation Result supersedes the old one.
...
1. Bob receives the Disclosure amendment notification.
2. Clicking the notification takes him to Alice's Claim on his canvas.
3. Bob initiates an evaluation from the Claim (or from the existing Evaluation Result). Platform requires the same Requirements Set as the previous evaluation. (Using a different Requirements Set produces a separate Evaluation Result, not a replacement.)
4. New Evaluation Result is created and linked to the Evaluation Agreement.
5. Previous Evaluation Result's `status` becomes `superseded` and `supersededBy` points to the new result.
6. Both results remain visible on both canvases; the superseded one has a `SUPERSEDED` label, dimmed styling, and does not contribute to health minibars.

### 11.4 Disclosure decline

Alice declines Bob's Disclosure + Evaluation Agreement request.

1. In the combined request modal, Alice selects "Decline" (fourth option alongside Full/Selective/Proof-Only).
2. Optional reason textarea.
3. Both provisional artifacts (Disclosure Agreement and Evaluation Agreement) are deleted.
4. Bob's provisional Claim node enters the "Disclosure Declined" state with Alice's reason (if provided).
5. Bob can dismiss the declined node (removes it entirely from his canvas).
...
### 11.6 Ownership transfer (Transferring process)
...
Seven phases. Each phase is a unit of autonomous work with explicit acceptance criteria. Claude Code should run with `xhigh` effort and use `/ultrareview` before declaring a phase complete.
...
- Use `/ultrareview` at the end of each phase before reporting completion.
...
*End of spec. Claude Code: begin Phase 1 when Andrew approves. Work to `xhigh` effort, surface ambiguities, run `/ultrareview` before declaring phase completion.*

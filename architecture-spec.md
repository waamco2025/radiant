...
#### 8.8.7 Forward pointers

- **Phase 17.1** — Buyer post flow: `makeRfp` lifecycle metadata, "Post RFP" entry point on the Directory chrome, RFP appears in the Directory immediately after posting.
- **Phase 17.2** — Supplier discovery + self-evaluation response: new `makeRfpResponse` factory; supplier sees RFP, self-evaluates against the listed RS(es), submits response.
- **Phase 17.2.1** — Buyer review surface + initiate formal Evaluation Agreement: buyer sees responses, picks one, opens the existing Request EA modal pre-populated with the response's Claim + RS context.

...
| `v22-ea-amendment` | EA grantee only | Grantor invokes `AmendEvaluationAgreementModal` (Phase 11E.1, §11.2a) | Pans to the affected Claim and opens the EA Detail Panel directly with the new Amendments section visible. | Dismisses on click. |
...
The Claim owner unilaterally amends the Evaluation Agreements they have granted. Two amendable surfaces:

- **`terms.evaluationDeadline`** — the EA's expiration date.
- **The underlying Claim's `acknowledgments[]`** — see Option B note below.

Per platform invariant, only the Claim owner (= EA grantor) can amend. The Evaluator (= EA grantee) is notified informationally; their recourse if they object to the amended terms is to revoke the EA. Amendments to one EA do not affect the paired Disclosure Agreement.

The amendment is recorded on the EA's `amendments[]` array (mirrors §11.2 DA shape). Each entry carries:

- `date` — ISO timestamp.
- `note` — the grantor's optional free-text amendment note.
- `termsBefore` — `{ evaluationDeadline }` snapshot of the prior value.
- `acknowledgmentChanges` — `{ added: [...], removed: [...], edited: [...] }` delta computed by `diffAcknowledgments(prevAcks, nextAcks)`.

A single `v22-ea-amendment` notification fires on the EA grantee's inbox (§7.4); click pans to the Claim and opens the EA Detail Panel directly.

#### Acknowledgments — Option B (prototype) vs Option C (production)

The prototype ships **Option B**: acknowledgments live on the Claim. Editing acknowledgments via Amend EA mutates the Claim's `acknowledgments[]` directly. The EA's `acknowledgmentsAccepted` audit-trail field is preserved as historical truth (what the Evaluator originally accepted at request time — that snapshot doesn't change post-hoc).

Option B has known limitations:

1. **Multi-EA implicit propagation.** When a Claim has Evaluation Agreements to multiple grantees, editing acknowledgments via one EA silently changes the underlying state for all of them. Only the targeted EA's grantee receives a `v22-ea-amendment` notification. Other grantees on the same Claim see their EA's effective acknowledgment terms change without explicit notice.
2. **Lineage chaining in the EA Amendments section.** Each amendment record stores `termsBefore.evaluationDeadline` for diffing against the *current* EA terms, but for amendments beyond the most recent the "before → after" display is not chained through prior amendments — so older entries show their `termsBefore` against the present-day deadline rather than the deadline that was current at the time of that amendment.

**Option C is the production target:**

- The EA carries a per-EA snapshot of acknowledgments (`acknowledgmentsAtTimeOfRequest: [{ id, title, description }]`) separate from the live Claim state. The Claim's `acknowledgments[]` becomes the *current default* used for new EAs; existing EAs reference their own snapshot.
- Amend EA modifies the EA's snapshot directly (no Claim mutation).
- All current EA grantees on the Claim are notified when their snapshot is amended.
- Amendment lineage is chained — each `termsBefore` references the immediately prior amendment's `termsAfter`.

Production migration: snapshot the current Claim's acknowledgments onto each existing EA at migration time; subsequent amendments operate on the snapshot.

> **Prototype note — Evaluation Agreement amendment.** The prototype writes the amended EA artifact (`makeAmendedEvaluationAgreement`) into `v22Provisionals.evaluationAgreements` and, when acknowledgments changed, atomically writes the updated Claim into `v22Provisionals.claims`. A single `v22-ea-amendment` notification is enqueued on the grantee's `addedRequests`. **In production:** EA amendment is an SDP lifecycle event producing a cryptographically signed amendment record linked to the existing EA; per Option C, the acknowledgment snapshot lives on the EA itself and SDP fans out notifications to every current grantee. The Claim mutation in Option B is a prototype simplification; it does not exist in production. **Authority:** SDP (amendment lifecycle, multi-grantee fan-out, snapshot management); Platform (notification dispatch).

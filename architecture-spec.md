...
### 11.2a Evaluation Agreement amendment (Phase 11E.1)

**Major revision in Phase 11.6 (#164): amendment-as-proposal.** EA amendments are now bilateral proposals requiring grantee acceptance, not unilateral grantor mutations. The Phase 11E.1 unilateral Option B model — where the grantor edited acknowledgments and the Claim mutated immediately, with the grantee receiving only an informational notification — was an exploit vector: the grantor could append commitments post-acceptance with no consent gate. The new model treats every EA amendment as a proposal that the grantee must explicitly accept or reject before any change takes effect.

Two amendable surfaces (unchanged):

- **`terms.evaluationDeadline`** — the EA's expiration date.
- **The underlying Claim's `acknowledgments[]`** — still owned by the Claim under Option B (Option C migration in production). Pre-acceptance, neither the EA's terms nor the Claim's acknowledgments mutate.

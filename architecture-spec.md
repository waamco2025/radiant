...
8. If the result is unsatisfactory, Alice can amend her Claim (add more Assets) and amend the Disclosure Agreement with Bob to include those new Assets. Bob gets `v22-claim-amendment` + `v22-da-amendment` notifications (§7.4), re-runs the evaluation. Same Req Set + different evidence produces a new result that supersedes the old (§11.3).
...
| `v22-da-amendment` | Grantee of the amended DA | Grantor submits `AmendDisclosureModal` | Pans to the affected Claim and opens the DA Detail Panel directly via `setOpenAgreement({ kind: 'disclosure', disclosureAgreementId })`. Badge renders as `DA AMENDED` (indigo). | Dismisses on click or on the V2.1 inbox's standard dismissal UI. |
| `v22-claim-amendment` | Every counterparty with an active DA on the amended Claim (fan-out, deduped by party) | Claim owner submits `AmendClaimModal` | Pans to the Claim and opens its node Detail Panel via `setSel(claimId)`. Badge renders as `CLAIM AMENDED` (indigo). | Dismisses on click. |
...
> **Prototype note — Claim amendment.** The prototype writes the `amendments[]` entry directly to the Claim artifact in `v22Provisionals` on submit. **Phase 11E.2 ship (#102):** Claim amendments now fire a `v22-claim-amendment` notification to every counterparty with an active DA on the affected Claim (fan-out, deduped by party; "active" = not provisional, not declined, not revoked, not expired). Click pans to the Claim and opens its node Detail Panel — see §7.4. **In production:** Claim amendments are DPP lifecycle events on the existing Claim artifact; each amendment is cryptographically recorded and the amendment chain is verifiable on-chain. Counterparty notifications are emitted by DPP as lifecycle events. **Authority:** DPP (amendment registration + event emission).
...
1. Bob receives `v22-claim-amendment` + `v22-da-amendment` notifications (§7.4).
...
- **§7.4 Notification table + §11.2 prototype notes — Phase 11E.2 (#102):** reciprocal cross-role notifications wired for the original two amendment flows (Claim + DA), closing the long-standing convention violation flagged in CLAUDE.md (every party-to-party state change must fire reciprocal notifications). Three deliverables: (1) the existing `v22-amendment` (DA-amendment) notification type was renamed to `v22-da-amendment` so it parallels `v22-ea-amendment` and the new `v22-claim-amendment`; click handler updated to deep-link directly to the DA Detail Panel via `setOpenAgreement({ kind: 'disclosure', disclosureAgreementId })`, mirroring the EA amendment routing instead of the prior pan-only behavior. Inbox badge label updated `AMENDED` → `DA AMENDED`. (2) New `v22-claim-amendment` type fires from `handleV22AmendClaimSubmit` to every counterparty with an *active* DA on the affected Claim (fan-out, deduped by party; "active" = not provisional, not declined, not revoked, not expired). Click pans to the Claim and opens its node Detail Panel; badge reads `CLAIM AMENDED`. (3) Notification body copy now includes the Claim name + optional `(Note: …)` suffix for both new types. Pre-fix the DA amendment row body fell through to a bare `req.asset?.name` (no context); the Claim amendment had no notification at all. §7.4 notification table + §11.2 prototype notes updated; §6 worked example + §11.3 evaluation-amendment narrative now reference both new types together.

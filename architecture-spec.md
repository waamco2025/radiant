...
>
> Round 17 status: **Phase 16.2.6.4 shipped** (label z-order by cluster size — smaller clusters render on top; RFP owner-labels always topmost via `Z_RFP_LABEL = 1100`). Directory still hosts ~22,824 dots across 77 Actors. Next: Phase 17.0–17.2 (RFP MVP — factory promotion, supplier discovery + response, buyer review). Polish-backlog #200 (Directory mini-card swap) and #201 (RFP cluster expansion at bottom third) are deferred-future candidates.

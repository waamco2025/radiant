...
- **Footer version:** v0.16.1.5 (rolled forward during Phase 16.1.5).

- **Last shipped phase:** **Phase 16.1.5 — Hotfix: Frustum culling + raycast bounding-sphere fix.** Two QA issues from Phase 16.1.4, both downstream of that phase's lifecycle change: (1) dots disappeared at zoom > ~60%; (2) dot clicks no longer opened the Detail Panel. **Shared root cause**: `THREE.InstancedMesh.boundingSphere` is auto-computed from underlying geometry vertices, which sit at world origin because instance positions live in per-instance matrices, not the geometry. The first computed sphere is tiny (radius = `DOT_RADIUS = 3`, centred at origin) and cached on the mesh. Both the renderer's frustum check and the raycast pre-filter (`InstancedMesh.raycast` → `raycaster.ray.intersectsSphere(_sphere)`) read this cached sphere — the tiny origin-sphere fell outside the camera frustum at high zoom and outside click rays at any zoom, so the entire mesh was rejected before per-instance work could run. Phase 16.1.4's lifecycle stabilization didn't touch bounding-sphere caching, so both symptoms surfaced once the lifecycle stopped masking them. **Fix**: on each InstancedMesh (dots + Actor squares + RFP), `frustumCulled = false` + `boundingSphere = new THREE.Sphere(origin, Infinity)`. The brief's alternate Item 2 "closure staleness" diagnosis didn't match actual code (handlers are React JSX props with `useCallback` deps, not `addEventListener` in a long-lived effect) and no handler refactor was required.
...
- **Active phase queue:** Round 16 closed at Phase 16.1.5. **Round 17 plan:** (1) **Phase 16.2** — Directory Layer seed expansion (~12 mock supplier Actors to validate spatial layout + InstancedMesh at scale); (2) **Phase 17.0** — RFP factory promotion + buyer post flow + clickable RFP dots + RFP Detail Panel; (3) **Phase 17.1** — Supplier discovery + self-evaluation response flow (new `makeRfpResponse` factory); (4) **Phase 17.2** — Buyer review surface + initiate formal Evaluation Agreement. After Round 17: Detail Panel cleanup (#58, #104, #116, #161, #180) → Cascading Disclosures (#26) → Netgraph cleanup (#4, #130) → Search/aggregate (#27) → Network Event Log (#30). See `ROUND-17-CONTEXT.md` for per-phase load-in lists.
...
The full per-phase completion log lives in [`CLAUDE-phase-log.md`](./CLAUDE-phase-log.md). It contains every phase note from Phase 1 (V2.2 data model foundation, 2026-04-17) through Phase 16.1.5 (frustum culling + raycast bounding-sphere fix, 2026-05-10), in chronological order.

...
For historical phase notes (every per-phase completion entry from Phase 1 through Phase 12.3), see [`CLAUDE-phase-log.md`](./CLAUDE-phase-log.md). For the architectural model, see [`architecture-spec.md`](./architecture-spec.md). For the open / partial / deferred work queue, see [`polish-backlog.md`](./polish-backlog.md). For the next-conversation foundation upload checklist, see [`ROUND-13-CONTEXT.md`](./ROUND-13-CONTEXT.md).
...
- **Footer version:** v0.12.3 (bumped during Phase 12.3).
- **Last shipped phase:** Phase 12.3 — Multi-RS UX Refinements + Asset Removal Bug Fix (3 bug fixes + 2 UX pivots).
- **Active phase queue:** Phase 12.3 shipped (Bug A RS picker dedup + Bug B Asset Remove propagation + Bug C Supersession link + Pivot 1 checkbox picker + Pivot 2 grouped review rows; #170 filed; footer v0.12.3). Phase 12.4 candidate: Asset file viewer in Run Evaluation left panel (carryover from V2.1, awaiting Andrew confirmation). **Phase 13+ priority order (boundaries TBD, strict ordering):** (1) PoE nodes #168 → (2) Badges #169 → (3) Directory Layer #43, #45, #47, #132, #162, #163 → (4) AI Shopper #47 (overlaps), #123 → (5) Cascading Disclosures #26 → (6) Detail Panel UI cleanup #58, #104, #116, #161+ → (7) Netgraph cleanup #4, #130 → (8) Search/aggregate #27 → (9) Network Event Log #30. Phase 11.6 deferred follow-up: #167 (DA amendment-as-proposal model — extension of #164). Phase 12.3 follow-up: #170 (consolidate `assetReferences[]` / `referencedAssetIds[]`).
...
The full per-phase completion log lives in [`CLAUDE-phase-log.md`](./CLAUDE-phase-log.md). It contains every phase note from Phase 1 (V2.2 data model foundation, 2026-04-17) through Phase 12.3 (Multi-RS UX Refinements, 2026-05-04), in chronological order.

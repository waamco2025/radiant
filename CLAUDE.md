...
- **Footer version:** v0.16.2.0 (rolled forward during Phase 16.2; held at v0.16.2.0 across the 16.2.1 / 16.2.2 / 16.2.3 / 16.2.4 / 16.2.5 / 16.2.5.1 backtrack-hotfixes).

- **Last shipped phase:** **Phase 16.2.5.1 — Smooth cluster dot geometry.** One-line bump: cluster-dot `CircleGeometry` segment count 16 → 64 in `DirectoryLayer.jsx`. With Phase 16.2.5's `DOT_RADIUS = 20.4` (7× pre-16.2.5), 16-sided dots were visibly polygonal at zoom > ~100%. 64 segments produces a perceptually smooth circle at every supported zoom up to MAX_ZOOM = 4.0. Per-frame cost negligible. RFP hollow circles (separate code path) untouched. Footer stays at v0.16.2.0 per backtrack-hotfix convention.
...
The full per-phase completion log lives in [`CLAUDE-phase-log.md`](./CLAUDE-phase-log.md). It contains every phase note from Phase 1 (V2.2 data model foundation, 2026-04-17) through Phase 16.2.5.1 (smooth cluster dot geometry, 2026-05-16), in chronological order.

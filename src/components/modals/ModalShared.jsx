...
export function InfoRow({ label, value, mono }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', minHeight: 34, borderBottom: '1px solid var(--border)' }}>
      <div style={{ width: 140, flexShrink: 0, fontSize: 12, color: 'var(--text-dim)', paddingLeft: 4 }}>{label}</div>
      <div style={{ flex: 1, fontSize: 12, fontFamily: mono ? 'var(--font-mono)' : 'var(--font-display)', color: 'var(--text-tertiary)' }}>{value}</div>
    </div>
  )
}

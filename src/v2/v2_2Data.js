...
export function deriveAgreementEdges(view) {
  ...
  for (const da of allDasForEdges) {
    ...
    if (kind === 'parseResult' && internal && hasScopeAssets) {
      // Phase 20.0 (Change B): Parse Results are buried on the source Asset's
      // child layer, so the Parse Result → source Asset Full edge no longer
      // renders on the parent canvas. The child-layer edge is generated by
      // V2Canvas's `handleDive` (anchor Asset → Parse Result) from the
      // synthesized `children` array. The DA itself stays in the data model
      // (Parse Result Detail Panel rows, audit trails, future Cascading
      // Disclosures still read it) — only the parent-layer edge is suppressed.
      continue
    }
...
function assignChainSlots(view) {
  ...
  for (const claim of view.claims || []) {
    ...
    // Phase 20.0 (Change D): Parse Results no longer count toward a chain's
    // eval activity. After burial they don't live on the parent canvas, so
    // they don't occupy the chain's eval row and shouldn't pull the chain
    // to the full SLOT_HEIGHT pitch. `evalNodes` is now {ERs targeting the
    // Claim} ∪ {PoEs wrapping those ERs} only. Consequence: a Claim whose
    // only eval activity was a Parse Result (e.g. Dave's claim-chipco-prm-ic)
    // flips from a 600-pitch eval chain to a 200-pitch lean chain.
...
export function buildV22Canvas(view) {
  ...
  ownedAssets.forEach((asset) => {
    ...
    // Phase 20.0 (Change C): synthesize a child-layer `children` array of
    // this owned Asset's Parse Results. Parse Results no longer render on
    // the parent canvas (Change A) — they're buried as children, revealed
    // by diving into the Asset (V2Canvas `handleDive` reads `node.children`
    // + the legacy parse-child edge branch). Source of truth is
    // `view.parseResults.filter(sourceAssetId === asset.id)`, NOT
    // `asset.parseResultIds` (an aggregate that can lag in provisional
    // states — design doc §9). Only OWNED Assets get children; pulled
    // Assets stay non-divable (no StackPeeks → dblclick falls through).
    const childPRs = view.parseResults.filter((pr) => pr.sourceAssetId === asset.id)
    node.children = childPRs.map((pr) => ({
      ...parseResultToNode(pr, 0, 0),
      sourceEvidenceId: pr.sourceAssetId, // V2.1 field name V2Canvas reads to build the anchor→PR edge
      isParse: true,                      // belt-and-suspenders (parseResultToNode already sets isParse + category:'parse')
    }))
    nodes.push(node)
  })
  ...
  // ── Owned Parse Results ──────────────────────────────────────────────
  // Phase 20.0: Parse Results are no longer parent-layer nodes. They are
  // buried as children of their source Asset and surface on the Asset's
  // child layer (entered via dive). The synthesized `children` array is
  // attached in the owned-Asset placement loop above (Change C); the
  // child-layer nodes + edges are generated by V2Canvas's `handleDive`.
  // The former parent-layer placement block (Phase 10.2.1) is removed.
  // `COL_OWN_PARSE` / `COL_OWN_PARSE_eff` remain defined (unused on the
  // parent canvas now) — their removal is a deferred cleanup.
